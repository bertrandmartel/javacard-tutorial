...
public class PasswordManagerPinTest extends JavaCardTest {
    ...
    private final static byte[] CMD_VERIFY_PIN_CODE = new byte[]{0x00, 0x20, 0x00, (byte) 0x80};
    ...
    private void verifyPinCode(byte[] data, int expectedSw, byte[] expectedResponse) throws CardException {
    ...
    public void duplicateAddTest() throws CardException {
        sendListId(new byte[]{}, 0x9000, new byte[]{});
        sendAddPassword(DATA_ENTRY_VALID.getFullApdu(), 0x9000, new byte[]{});
        sendAddPassword(DATA_ENTRY_VALID.getFullApdu(), PasswordPinManager.SW_DUPLICATE_IDENTIFIER, new byte[]{});
        sendAddPassword(DATA_ENTRY_VALID.getFullApdu(), PasswordPinManager.SW_DUPLICATE_IDENTIFIER, new byte[]{});
        sendListId(new byte[]{}, 0x9000, DATA_ENTRY_VALID.getId());
    }
    ...
    public void getInvalidIdTest() throws CardException {
        sendListId(new byte[]{}, 0x9000, new byte[]{});
        sendAddPassword(DATA_ENTRY_VALID.getFullApdu(), 0x9000, new byte[]{});
        sendGetPassword(DATA_ENTRY_VALID1.getId(), PasswordPinManager.SW_IDENTIFIER_NOT_FOUND, new byte[]{});
    }
    ...
    public void deleteIdInvalidTest() throws CardException {
        sendListId(new byte[]{}, 0x9000, new byte[]{});
        sendAddPassword(DATA_ENTRY_VALID.getFullApdu(), 0x9000, new byte[]{});
        sendDeletePassword(DATA_ENTRY_VALID1.getId(), PasswordPinManager.SW_IDENTIFIER_NOT_FOUND, new byte[]{});
    }
